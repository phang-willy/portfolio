...
export const contactSubmissionBodySchema = contactFormSchema.extend({
  ...
  /**
   * Honeypot anti-spam. Le champ doit rester vide côté navigateur humain.
   * S'il est rempli, le serveur accepte la requête mais ne l'envoie pas.
   */
  website: z.preprocess(
    (val) => (typeof val === "string" ? val.trim() : ""),
    z.string().max(500, "Soumission invalide."),
  ),
});

export type ContactSubmissionBody = z.infer<typeof contactSubmissionBodySchema>;

export function isHoneypotFilled(value: ContactSubmissionBody["website"]) {
