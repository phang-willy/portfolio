...
@Component({
  selector: 'app-experience-contract-types-list-page',
  host: { class: 'block' },
  imports: [
    AdminDatePipe,
  ...
  templateUrl: './experience-contract-types-list-page.html',
})
export class ExperienceContractTypesListPage {
  ...
  protected readonly items = signal<readonly ExperienceContractTypeAdminListItem[]>([]);
  ...
  protected readonly deleteTarget = signal<ExperienceContractTypeAdminListItem | null>(null);
  protected readonly deactivateTarget = signal<ExperienceContractTypeAdminListItem | null>(null);
  protected readonly reactivateTarget = signal<ExperienceContractTypeAdminListItem | null>(null);
  ...
  protected readonly filteredItems = computed(() => {
    ...
    if (query) {
      rows = rows.filter((item) => {
        const haystack = [item.slug, item.codeFr, item.codeEn, item.titleFr, item.titleEn]
          .join(' ')
          .toLowerCase();
  ...
  protected readonly pageReport = computed(() => {
    ...
    return `Showing ${showing} of ${total} contract types`;
  });
  ...
  protected confirmDelete(item: ExperienceContractTypeAdminListItem): void {
  ...
  protected confirmDeactivate(item: ExperienceContractTypeAdminListItem): void {
  ...
  protected confirmReactivate(item: ExperienceContractTypeAdminListItem): void {
  ...
  protected acceptReactivate(): void {
    ...
    this.contractTypeService
      .reactivateContractType(item.id, this.reactivateForm.getRawValue())
      .pipe(
        catchError(() => {
          this.errorMessage.set('Unable to reactivate the contract type.');
          return EMPTY;
        }),
  ...
  protected acceptDeactivate(): void {
    ...
    this.contractTypeService
      .deactivateContractType(item.id, this.deactivateForm.getRawValue())
      .pipe(
        catchError(() => {
          this.errorMessage.set('Unable to deactivate the contract type.');
          return EMPTY;
        }),
        takeUntilDestroyed(this.destroyRef),
      )
  ...
  protected acceptDelete(): void {
    ...
    this.contractTypeService
      .deleteContractType(item.id, this.deleteForm.getRawValue())
      .pipe(
        catchError(() => {
          this.errorMessage.set('Unable to delete the contract type.');
          return EMPTY;
        }),
        takeUntilDestroyed(this.destroyRef),
      )
  ...
  private loadItems(): void {
    ...
    this.contractTypeService
      ...
      .pipe(
        catchError(() => {
          this.errorMessage.set('Unable to load contract types.');
          return EMPTY;
        }),
